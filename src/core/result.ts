...
export class Result<S, P, C> {
     ...
     * This will represent the time it took to rehydrate the result when the result is deserialized.
     */
     ...
     * Multiple errors can be captured, reflecting various issues that occurred during the action's execution.
     *
     * @returns An array of errors, if any were encountered during the execution of the action.
     */
    get errors(): Error[] {
        return this._errors;
    }
    ...
    /**
     * Provides access to the state of the system before the action was executed.
     *
     ...
     *
     * @returns The time at which the result was created in milliseconds.
     */
    get timestamp(): Date {
     ...
     *
     * @returns The execution time of the action in milliseconds.
     */
    ...
    /**
     * Provides access to the metadata attached to the result.
     *
     ...
     * @param prevState The state before the action was applied.
     * @param nextState The state after the action was attempted, usually null as the state will likely remain unchanged.
     *
     ...
     *
     * @hook `before-deserialize-state` Invoked before the deserialization of state begins. Receives the previous and next state objects from the JSON as arguments.
     * @hook `after-deserialize-state` Invoked after the deserialization of state is completed. Receives the deserialized previous and next state objects as arguments.
     *
    ...
    public static async fromJSON<S, P, C>(json: IResultJSON<S, P, C>): Promise<Result<S, P, C>> {
        ...
        await Hooks.invoke("before-deserialize-state", json.prevState, json.nextState);

        let nextState = json.nextState;
        let prevState = json.prevState;

        await Hooks.invoke("after-deserialize-state", prevState, nextState);
